...
public class VpnPlugin extends Plugin {
    ...
    public void getVpnLogs(PluginCall call) {
        // Ensure internal log path is initialized (critical after crash restart)
        L.init(getContext());
        try {
            // L.getBufferedLogs() automatically handles:
            // - In-memory buffer (current session, fast)
            // - File-based recovery (after crash, reads from internal storage)
            String logs = L.getBufferedLogs();
            
            JSObject ret = new JSObject();
            ret.put("logs", logs);
            call.resolve(ret);
        } catch (Throwable e) {
            // Last resort: try to read from file directly
            try {
                String fileLogs = L.readLogsFromFile();
                JSObject ret = new JSObject();
                ret.put("logs", fileLogs != null ? fileLogs : "Error reading logs: " + e.getMessage());
                call.resolve(ret);
            } catch (Throwable e2) {
